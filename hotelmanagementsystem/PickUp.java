	...
	public PickUp() throws SQLException {
		...
		contentPane.add(lblTypeOfCar);
                ...
                try{
                    ...
                    ResultSet rs = c.smtStatement.executeQuery("select * from driver");
                    while(rs.next()){
                        c1.add(rs.getString("comp_model"));    
                    }
                }catch(Exception e){ }
...
//		contentPane.add(lblInfo);
		...
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String SQL = "select * from driver where comp_model = '"+c1.getSelectedItem()+"'";
				try{
				
					Connectivity c = new Connectivity();
					rs = c.smtStatement.executeQuery(SQL);
